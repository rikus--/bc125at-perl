...
sub _setup_widgets {
    ...
    my $scroll = Gtk2::ScrolledWindow->new(Gtk2::Adjustment->new(
400, 200, 800, 1, 1, 50)
);
    ...
    for (
        _button("Read from scanner", sub {
            $self->populate_table(
                $self->{scanner}->get_all_channel_info()
            );
        }),
        _button("Write to scanner",  sub {
            $self->{scanner}->write_channels(
                undef,
                $self->harvest_table()
            );
        })
